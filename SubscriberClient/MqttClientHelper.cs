...
namespace PKTI.Persistence.Services
{
    public class MqttClientHelper
    {
        ...
        private TaskCompletionSource<bool> koddatMatchedTcs;
        private int type;
        // Используем для ожидания сообщения с нужным koddat

        // Конструктор для настройки клиента
        public MqttClientHelper(string URL, int port, byte koddat, string topic, int type)
        {
            this.koddat = koddat;
        ...
        public async Task Opros()
        {
            ...
            if (type == 1)
            {
                data = ProtocolData.CreateRandom(koddat, 1).ToString(); /*"K:04;F:01;Q:31;B:3562;A:01;R:42BB06;T:25.02.14,10:57:20";*/
            }
            else if (type == 2)
            {
                data = $"K:{koddat};C:074A;M:42BB9060DA1242BB906BDB12";
            }
